...
def render_connection_list(session_state):
    ...
    all_conns = {
        conn.id: conn for conn in session_state.network.get_all_connections(recurse=True)
        if search.lower() in conn.id.lower() or 
            search.lower() in type(v).__name__.lower() or
            (hasattr(conn, "source") and conn.source and search.lower() in conn.source.id.lower()) or
            (hasattr(conn, "destination") and conn.destination and search.lower() in conn.destination.id.lower())
    }
    nested_conns = remove_keys(all_conns, filtered_conns)
    st.subheader("Nested Connections")
    if nested_conns:
        for conn_id, conn_obj in nested_conns.items():
            conn_type = type(conn_obj).__name__
            with st.expander(f"**{conn_id}** ({conn_type})"):
                st.write(f"**Type:** {conn_type}")
        ...
        st.info("No nested connections yet. Add one to get started!")
...
def render_connection_form(session_state):
    ...
    if conn_type == "Pipe":
        ...
        # Friction
        friction_val = st.text_input(
            "Friction Coefficient", value=str(existing_conn.friction_coeff) if existing_conn and hasattr(existing_conn, "friction_coeff") and existing_conn.friction_coeff else "")
        if friction_val:
